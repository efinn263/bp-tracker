...
const DB = (() => {
    ...
    return new Promise((res, rej) => {
      ...
      req.onupgradeneeded = e => {
        ...
        if (!d.objectStoreNames.contains('medEvents')) {
          const s = d.createObjectStore('medEvents', { keyPath: 'id' });
          s.createIndex('ts', 'timestamp');
        }
      };
  ...
  function getRange(store, start, end) {
    return open().then(d => new Promise((res, rej) => {
      ...
      const rng  = (start && end) ? IDBKeyRange.bound(+start, +end)
                 ...
                 : end            ? IDBKeyRange.upperBound(+end)
                 : null;
  ...
  return {
    ...
    async exportAll() {
      const [readings, medEvents] = await Promise.all([
        this.getReadings(), this.getMedEvents()
      ]);
      return { version: 1, exportedAt: new Date().toISOString(), readings, medEvents };
    },
